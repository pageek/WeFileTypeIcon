...
//  RTX
//
//  Created by raindong on 15/10/25.
//  Copyright (c) 2015年 yjl. All rights reserved.
//
...
NSImage* WEWGetDefaultFileTypeIcon() {
    return [NSImage imageNamed:@"icon_file_normal"];
}
...
NSImage *WEWGetFileIcon(NSString *fileName){
    ...
    if (WEWIsWordFile(extension)) {
        ...
        return [NSImage imageNamed:@"icon_file_browser"];
    } else if (WEWIsKeynoteFile(extension)) {
        return [NSImage imageNamed:@"icon_file_keynote"];
    } else if (WEWIsNumbersFile(extension)) {
        return [NSImage imageNamed:@"icon_file_numbers"];
    } else if (WEWIsPageFile(extension)) {
        return [NSImage imageNamed:@"icon_file_page"];
    }
